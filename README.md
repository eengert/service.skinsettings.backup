...
1. In Kodi, enable installation from unknown sources if needed, then **Add-ons → Install from zip file**, selecting `service.skinsettings.backup-1.0.11.zip`.
...
Activating the restored skin makes Kodi load the complete transactionally restored `settings.xml`; the add-on waits for and verifies every live value before rebuilding AF3. AF3's generated Skin Variables include files require one final reload. The add-on verifies the imported live settings and generated XML immediately before that synchronous reload, then verifies the settings and restored helper files again before reporting success. It does not reset the skin or replay settings one at a time; a rejected per-setting API call can otherwise leave the skin blank after the reset has already happened.
